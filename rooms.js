...
export async function createRoom(roomName, minPlayers = 2, aiBots = 0, creatorPlayer = null) {
  ...
  if (!hasAccess) {
    throw new Error('Multiplayer access required. Please purchase multiplayer access to create rooms.');
  }
  ...
  const roomData = {
    ...
    isOpen: true, // Room is open for joining
    gameStarted: false, // Game hasn't started yet
    lastUpdated: new Date().toISOString() // For staleness tracking
  };
...
export async function joinRoom(roomId, player) {
  ...
  const hasAccess = await multiplayerPaywall.hasMultiplayerAccess();
  if (!hasAccess) {
    throw new Error('Multiplayer access required. Please purchase multiplayer access to join rooms.');
  }
  ...
  const maxPlayersInRoom = data.maxPlayers || data.minPlayers;
...
export async function startGame(roomId) {
  ...
  console.log('🎮 Room status updated to in_progress and marked as closed');
  return gameRoomId;
}
...
export async function getAvailableRooms() {
  ...
  try {
    ...
    console.log(`Optimized room query found ${snap.docs.length} recent rooms`);
    ...
    const validRooms = snap.docs
      ...
      .filter(room => {
        ...
        const hasCapacity = playerCount < (room.maxPlayers || room.minPlayers || 2);
        if (!hasCapacity) {
          console.warn(`Filtering out full room: ${room.roomName} (${playerCount}/${room.maxPlayers})`);
          return false;
        }
    ...
    console.log(`Returning ${validRooms.length} valid available rooms`);
    return validRooms;
    
  } catch (error) {
    console.error('Optimized room query failed, falling back to basic query:', error);
    ...
    const validRooms = snap.docs
      .map(d => ({ id: d.id, ...d.data() }))
      .filter(room => {
        if (!room.roomName || !room.hostUid) return false;
        const playerCount = room.players ? room.players.length : 0;
        if (playerCount === 0) return false;
        return playerCount < (room.maxPlayers || room.minPlayers || 2);
      })
      .slice(0, 20); // Limit to prevent overload
    
    console.log(`Fallback query returning ${validRooms.length} rooms`);
    return validRooms;
  }
}

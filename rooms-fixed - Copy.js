...
import { collection, addDoc, updateDoc, doc, getDoc, getDocs, query, where, onSnapshot, arrayUnion, writeBatch } from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js';

let db = null;
let roomsCollection = null;

// Initialize Firebase and collection references
function getDb() {
  if (!db) {
...
function getRoomsCollection() {
  if (!roomsCollection) {
    ...
    if (firestoreDb) {
      roomsCollection = collection(firestoreDb, 'rooms');
    }
  }
...
export async function joinRoom(roomId, player) {
  ...
  const ref = doc(firestoreDb, 'rooms', roomId);
...
export async function startGame(roomId) {
  ...
  const { roomId: gameRoomId } = await createGameRoom(players[0].displayName, players.length + aiBots, aiBots);
...
export async function clearLobbyRooms() {
  const roomsCollection = getRoomsCollection();
  const firestoreDb = getDb();
  if (!roomsCollection || !firestoreDb) {
